...
export const getStaticProps: GetStaticProps = async ({ params }) => {
	const [favorites, categories] = await Promise.all([
		client.getEntries<Favorite>({ content_type: 'storefrontFavorite' }),
		client.getEntries<Category>({ content_type: 'storefrontCategory' }),
	]);
...
export default function Example({ categories, favorites }: Props) {
	...
	return (
		<div className='bg-white relative'>
			...
			{/* Top navigation */}
			<nav aria-label='Top' className='sticky top-0 z-20 bg-white bg-opacity-70 backdrop-filter backdrop-blur-xl'>
